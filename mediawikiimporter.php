...
class MediaWikiImporter {
	static function removeSectionsDOMDocument(DOMNode $node) {
		$markForRemove = false;
		$indicesToRemove = [];
	...
	static function removeSections(\PHPHtmlParser\Dom\Node\HtmlNode $node) {
		$markForRemove = false;
		$indicesToRemove = [];
    ...
    static function fixUrlsDOMDocument(DOMNode $node, $urlPrefix) {
	...
	static function fixUrls($node, $urlPrefix) {
	...
	static function removeAllAttributes($contentDiv) {
	...
	static function importPage($page) {
		...
		$html = preg_replace('~.*<div class="mw-parser-output"><p>~s', '<div class="mw-parser-output">', $html);
		...
		self::fixUrls($contentDiv, '/' . pathinfo($page, PATHINFO_DIRNAME));
